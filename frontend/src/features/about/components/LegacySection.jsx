"use client";
import React, { useState, useEffect } from 'react';
import { motion } from 'framer-motion';
import api from '../../../api/axios';
import { DEFAULT_LEGACY } from '../../admin/cms/constants/defaultCmsData';

const LegacySection = ({ previewData }) => {
  const [data, setData] = useState(previewData || DEFAULT_LEGACY);

  useEffect(() => {
    if (previewData) {
      setData(prev => ({ ...prev, ...previewData }));
      return;
    }
    const fetchData = async () => {
      try {
        const response = await api.get('/cms/legacy', { hideLoader: true });
        if (response.data) {
          setData(prev => ({ ...prev, ...response.data }));
        }
      } catch (error) {
        console.error('Error fetching Legacy data:', error);
      }
    };
    fetchData();
  }, [previewData]);

  if (data?.showSection === false && !previewData) return null;

  return (
    <section className="py-20 bg-transparent">
      <div className="w-[98%] max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">

          {/* Left Image */}
          <motion.div
            initial={{ opacity: 0, x: -50 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: false, amount: 0.3 }}
            transition={{ duration: 0.8, ease: "easeOut" }}
            className="relative rounded-2xl overflow-hidden shadow-xl h-[400px] lg:h-[450px]"
          >
            <img
              src={data.image || "/assets/Images/image 2.png"}
              alt="KSBM Legacy"
              className="w-full h-full object-cover"
            />
          </motion.div>

          {/* Right Content */}
          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={{ once: false, amount: 0.3 }}
            variants={{
              hidden: { opacity: 0 },
              visible: {
                opacity: 1,
                transition: { staggerChildren: 0.2 }
              }
            }}
            className="flex flex-col"
          >
            <motion.span
              variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }}
              className="text-gray-400 text-xs font-semibold tracking-[0.2em] uppercase mb-4"
            >
              {data.subheading}
            </motion.span>
            <motion.h2
              variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }}
              className="text-3xl lg:text-4xl font-bold text-primary mb-4 leading-snug"
            >
              {data.heading}
            </motion.h2>
            <motion.div
              variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }}
              className="text-gray-600 text-sm md:text-base leading-relaxed space-y-2 text-justify"
            >
              {data.description?.split('\n').filter(p => p.trim()).map((para, idx) => (
                <p key={idx}>{para}</p>
              ))}
            </motion.div>
          </motion.div>

        </div>
      </div>
    </section>
  );
};

export default LegacySection;
