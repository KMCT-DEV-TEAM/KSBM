    ...
    if (isPreviewModalOpen && iframeRef.current) {
      const sendData = () => {
        if (iframeRef.current && iframeRef.current.contentWindow) {
          iframeRef.current.contentWindow.postMessage({ type: 'preview-cms-data', componentName: 'AdvisoryBoard', payload: previewData }, '*');
        }
      };
    ...
    await confirmAction({
      ...
      action: async () => {
        ...
        if (member.image && !member.image.startsWith('blob:') && !member.image.startsWith('http') && member.image !== '/assets/Images/image 31.png') {
          try {
            await api.delete('/upload', { data: { fileUrl: member.image }, hideLoader: true });
          } catch (err) {
            console.warn('Skipped deleting member image:', err);
          }
        }
  ...
  return (
    <div className="space-y-6 w-full relative">
      <PageHeader
        ...
        description="Manage the members displayed in the Advisory Board section."
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 shrink-0">
              ...
              <button onClick={() => setIsAddModalOpen(false)} className="text-gray-400 hover:text-red-500 p-1 hover:bg-red-50 rounded-md transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3 p-5 border-t border-gray-100 bg-gray-50 shrink-0">
              <button 
                ...
                className="px-5 py-2 text-sm font-semibold text-gray-600 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-all"
              >
                Cancel
              </button>
              <button 
                ...
                className="flex items-center gap-2 px-6 py-2 text-sm font-semibold text-white bg-primary hover:bg-[#151c48] rounded-xl shadow-md transition-all disabled:opacity-50"
              >
                <Plus className="w-4 h-4" /> Add Member
              </button>
            </div>
...
export default ManageAdvisoryMembers;
