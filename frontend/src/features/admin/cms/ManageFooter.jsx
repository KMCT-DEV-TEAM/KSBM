      ...
      <SectionForm title="General & Social">
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <textarea rows={2} value={description} onChange={e => setDescription(e.target.value)} maxLength={150} className="w-full p-2.5 bg-white border border-gray-200 rounded-md text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <input type="text" value={contactInfo.address} onChange={e => setContactInfo({...contactInfo, address: e.target.value})} maxLength={100} className="w-full p-2.5 bg-white border border-gray-200 rounded-md text-sm outline-none" />
              <div className="text-right text-xs text-gray-400 mt-1">
                {contactInfo.address?.length || 0} / 100
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-semibold text-gray-500">Email</label>
              <input type="text" value={contactInfo.email} onChange={e => setContactInfo({...contactInfo, email: e.target.value})} maxLength={50} className="w-full p-2.5 bg-white border border-gray-200 rounded-md text-sm outline-none" />
              <div className="text-right text-xs text-gray-400 mt-1">
                {contactInfo.email?.length || 0} / 50
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-semibold text-gray-500">Phone</label>
              <input type="text" value={contactInfo.phone} onChange={e => setContactInfo({...contactInfo, phone: e.target.value})} maxLength={20} className="w-full p-2.5 bg-white border border-gray-200 rounded-md text-sm outline-none" />
              <div className="text-right text-xs text-gray-400 mt-1">
                {contactInfo.phone?.length || 0} / 20
              </div>
            </div>
          </div>
          <div className="space-y-2">
            <label className="text-xs font-semibold text-gray-500">Copyright Text</label>
            <input type="text" value={copyrightText} onChange={e => setCopyrightText(e.target.value)} maxLength={100} className="w-full p-2.5 bg-white border border-gray-200 rounded-md text-sm outline-none" />
            <div className="text-right text-xs text-gray-400 mt-1">
              {copyrightText?.length || 0} / 100
            </div>
          </div>
        </div>
      </SectionForm>
