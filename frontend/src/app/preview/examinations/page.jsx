...
export default function ExaminationsPreviewPage() {
  const [previewData, setPreviewData] = useState(null);

  useEffect(() => {
    const handleMessage = (event) => {
      if (event.data?.type === 'preview-examinations-data') {
    ...
    if (window.parent) {
      window.parent.postMessage({ type: 'iframe-ready', source: 'examinations' }, '*');
    }
  ...
  return (
    <div className="w-full min-h-screen bg-[#FCFCFD]">
      {previewData ? (
        <ExaminationsLanding previewData={previewData} />
      ) : (
